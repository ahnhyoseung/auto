            ...
            for sheet_name, eval_file in evaluator_files.items():
                ...
                try:
                    ...
                    for r_idx in range(1, ws_src.nrows):
                        ...
                        if not val:
                            continue
                        # 쉼표로 분리 후 "시험:" 포함된 항목 찾기
                        parts = [p.strip() for p in val.split(",")]
                        시험_part = next((p for p in parts if "시험:" in p), None)
                        if 시험_part:
                            cleaned = 시험_part.split("시험:", 1)[1].strip()
                            cleaned = re.sub(r'<[^>]+>', '', cleaned).strip()
                            cell.value = cleaned

                    # 불필요한 열 삭제
                    # 남길 열(원본): B(2)등록일, C(3)회사기관명, D(4)성명, F(6)평가사구분, J(10)시험유효, K(11)검사유효
                    # 삭제할 열 인덱스를 뒤에서부터 한 번에 처리
                    keep_cols = {2, 3, 4, 6, 10, 11}  # 남길 열 번호
                    total_cols = ws_eval.max_column
                    for col_idx in range(total_cols, 0, -1):
