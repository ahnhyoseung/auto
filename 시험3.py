...
def setup_driver():
    """Chrome 드라이버 초기화. 팝업 차단 해제 + 봇 감지 우회 옵션 포함."""
        ...
        try:
            ...
            for row_idx in range(1, ws_range.nrows):
                for col_idx in range(ws_range.ncols):
                    ...
                    if isinstance(val, str):
                        # 줄바꿈을 공백으로 변환 후 연속 공백 정리
                        val = val.replace('\n', ' ').replace('\r', ' ')
                        val = re.sub(r' +', ' ', val).strip()
                        # 제어문자 제거
                        val = re.sub(r'[\x00-\x08\x0b\x0c\x0e-\x1f]', '', val)
                        # 32767자 초과 시 잘라내기 (openpyxl 셀 한도)
                        if len(val) > 32767:
                            val = val[:32767]
            ...
            for i, sheet_name in enumerate(sheet_names_sorted):
                ...
                try:
                    ...
                    # D열 유효기간 종료일 확인 → 6개월 이내면 H열에 "x유효기간" 표시
                    from datetime import datetime
                    today = datetime.today()
                    six_months_later = today.replace(month=today.month + 6) if today.month <= 6 else \
                        today.replace(year=today.year + 1, month=today.month - 6)
                    for r_idx in range(2, ws_eval.max_row + 1):
                        d_cell = ws_eval.cell(row=r_idx, column=4)  # D열 = 유효기간
                        val = d_cell.value
                        if not val:
                            continue
                        try:
                            # "2025-02-17 ~ 2026-02-20" 형태에서 오른쪽 날짜 추출
                            if isinstance(val, str) and "~" in val:
                                end_str = val.split("~")[-1].strip()
